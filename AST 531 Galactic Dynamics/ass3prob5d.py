...
def rDot(angMomentum, radius):
	return -np.sqrt(2*(np.log(1 + radius)/radius - np.log(2) - angMomentum**2/2 * (1/radius**2 - 1)))
...
ax.set_xlabel('$\phi$')
ax.set_ylabel('$r$')
ax.set_title('NFW Orbital Parameters, apocenter $\~r=1$')
...
ax.set_xlabel('$x$')
ax.set_ylabel('$y$')
ax.set_title('NFW Orbits, apocenter $\~r=1$')
